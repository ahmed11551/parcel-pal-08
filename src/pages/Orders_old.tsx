...
export default function OrdersPage() {
  ...
  const { data: tasksData, isLoading } = useQuery({
    queryKey: ["myTasks", role],
    queryFn: () => api.getMyTasks(role === "sender" ? "sender" : role === "carrier" ? "courier" : undefined),
  });

  const tasks = tasksData?.tasks || [];

  // Map task statuses to order statuses for compatibility
  const statusMapping: Record<string, string> = {
    'active': 'pending',
    'assigned': 'carrier_selected',
