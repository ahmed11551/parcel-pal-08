...
5. Добавьте в `.env.production` на сервере:
   ```env
   SMS_PROVIDER=smsru
   SMSRU_API_ID=72E487A6-DFE3-ED3E-7C90-BD60FC7CBA72
   ```
   
   **Ваш API ID:** `72E487A6-DFE3-ED3E-7C90-BD60FC7CBA72`
   
   ⚠️ **Важно:** Не коммитьте `.env.production` в Git! Этот файл должен быть только на сервере.
...
### Вариант 2: SMSC.ru (Дешевле SMS.ru)
...
3. Добавьте в `.env.production`:
   ```env
   SMS_PROVIDER=smscru
   SMSCRU_LOGIN=ваш_логин
   SMSCRU_PASSWORD=ваш_пароль
   ```

**Плюсы**: Дешевле SMS.ru (от 1.8 руб/сообщение), хорошая доставляемость

### Вариант 3: GetSMS.online (Самый дешевый)

1. Зарегистрируйтесь на [getsms.online](https://getsms.online)
2. Получите API ключ
3. Добавьте в `.env.production`:
   ```env
   SMS_PROVIDER=getsms
   GETSMS_API_KEY=ваш_api_ключ
   ```

**Плюсы**: Самый дешевый (от 1.5 руб/сообщение)
...
1. Зарегистрируйтесь на [smsaero.ru](https://smsaero.ru)
2. Получите email и API ключ
3. Добавьте поддержку в `backend/src/utils/sms.ts` (по аналогии с SMSC.ru)

### Вариант 6: Другой провайдер
