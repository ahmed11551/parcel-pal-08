...
# Скопируйте весь этот скрипт на сервер и выполните: bash install-on-server-direct.sh
...
if [ "$EUID" -ne 0 ]; then 
    echo -e "${COLOR_RED}❌ Пожалуйста, запустите скрипт от root: sudo bash install-on-server-direct.sh${COLOR_NC}"
    exit 1
fi
